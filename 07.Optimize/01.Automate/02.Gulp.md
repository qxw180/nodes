...
var $ = gulpLoadPlugins({ pattern: '*', lazy: true }),
    _ = { app: 'app', dist: 'dist', test: 'test', 'cssPreProcessor': 'less' }; // 在此处切换less/sass

// 命令行传递参数
var knownOptions = {
    string: ['env', 'fix'],
    default: { env: process.env.NODE_ENV || 'production', fix: false }
};
...
gulp.task('dev:sass', function() {
    return gulp.src(_.app + '/sass/main.scss')
        .pipe($.plumber(swallowError))
...
gulp.task('dev:html', function() {
    return gulp.src([_.app + '/**/*.htm', '!' + _.app + '/tpls/**/*.htm'])
        .pipe($.plumber(swallowError))
        .pipe($.connect.reload());
});
...
gulp.task('dev:tmod', function() {
    return gulp.src([_.app + '/tpls/**/*.htm'])
        .pipe($.plumber(swallowError))
        .pipe($.tmod({
            templateBase: _.app + '/tpls',
            combo: true,
...
gulp.task('dev:watch', function() {
    if (_.cssPreProcessor === 'less') {
        gulp.watch([_.app + '/less/**/*.less'], ['dev:less']); //监听less
    } else if (_.cssPreProcessor === 'sass') {
        gulp.watch([_.app + '/sass/**/*.scss'], ['dev:sass']); //监听sass
    }
    gulp.watch([_.app + '/**/*.htm', '!' + _.app + '/tpls/**/*.html'], ['dev:html']);
    gulp.watch([_.app + '/tpls/**/*.htm'], ['dev:tmod']);
    gulp.watch([_.app + '/scripts/**/*.js'], ['dev:js', 'eslint']);
});

// 开启server
var httProxy = require('./httpProxy.js');
...
//    return gulp.src(_.app+'/styles/**/*.css')
//        .pipe($.cleanCss({compatibility: 'ie7'}))//不需要压缩可以注释掉
//        .pipe(gulp.dest(_.dist+'/styles/'));
//});

// 给css js html的引用 加hash值
gulp.task('build:rev', function() {
    return gulp.src(_.dist + '/index.htm')
        .pipe($.usemin({
            html: [$.rename('index_rev.htm')],
            css: [$.rev()],
            js: [$.rev()]
        }))
        .pipe(gulp.dest('dist/'));
...
gulp.task('build:index', function() {
    return gulp.src(_.app + '/index.htm')
        .pipe($.replace('<script src="/scripts/config.js"></script>', '<script src="scripts/main.js" ></script>'))
...
gulp.task('build:html', ['build:index'], function() {
    return gulp.src(_.app + '/includes/*.htm')
        .pipe($.htmlmin({ collapseWhitespace: true })) //不需要压缩的可以注释掉
...
gulp.task('build:sprites', function() {
    ...
    spriteOutput = gulp.src(_.app + '/styles/main.css')
    ...
    spriteOutput.css.pipe($.cleanCss({ compatibility: 'ie7' })) //不需要压缩可以注释掉
        .pipe(gulp.dest(_.dist + '/styles/'));
    spriteOutput.img.pipe($.imagemin({
            progressive: true, //针对jpg
            use: [$.imageminPngquant({ quality: '65-80', speed: 4 })] //png
        }))
        .pipe(gulp.dest(_.dist + '/images/sprites/'));
    return spriteOutput;
});
...
gulp.task('build:connect', function() {
    $.connect.server({
        root: _.dist,
        port: process.env.HAMMER_DEV_PORT || 8080,
        middleware: function() {
            return [$.connectSsi({
                baseDir: _.dist,
                ext: '.htm',
                domain: 'http://mini2015.qq.com/',
                method: 'readOnLineIfNotExist', // readOnLine|readLocal|readOnLineIfNotExist|
                onlineEncoding: 'GBK',
                localEncoding: 'utf8'
            })];
        }
...
gulp.task('build', function() {
    $.runSequence('build:del', 'build:sprites', ['build:js', 'build:img', 'build:html'], 'build:connect');
});

// ESLint
// 使用gulp eslint --fix true  或者直接eslint app/scripts --fix
gulp.task('eslint', function() {
    var fixFlag = options.fix;
    return gulp.src([_.app + '/**/*.js', _.test, '!node_modules/**', '!app/scripts/templateModule.js'])
        .pipe($.eslint({ fix: fixFlag })) // 实际上是在输出中fix
        .pipe($.eslint.format())
        ...
        .pipe(gulp.dest(function(data) {
            ...
            return data.base;
        })); // 将fix好的文件输出到源目录：overwrite all the original files.
});

// jsdoc3
gulp.task('doc', function(cb) {
    var config = require('./jsdoc.json');
    gulp.src(['README.md', _.app + '/**/*.js'], { read: false })
