...
JavaScript 引擎运行方式是先解析代码，获取所有被声明的变量，然后再一行一行的运行。
...
JS 引擎运行过程分为预编译期和执行期，JS 引擎是边运行边解析，在运行的过程中遇到**可执行代码**时就会首先创建一个**执行上下文**(Execution Context)，即编译。在生成执行上下文的过程会发生变量提升。
...
1. 参数处理
   1. 创建`arguments`对象
   2. 将函数参数添加到 VO 对象，并使用实参初始化(若实参未传入则初始化为`undefined`)
2. 函数声明处理：扫描函数内的函数声明并添加到 VO 对象，发现重复声明会覆盖变量对象已包含的同名属性
3. 变量声明处理：扫描函数内的`var`关键字声明的变量并添加到 VO 对象，初始化变量值为`undefined`，发现重复声明会忽略
4. 求出上下文内部`this`的值
...
JS 在执行阶段使用的变量都是来自 VO 对象，Review VO 的创建过程应该对变量提升有了更深入的理解。
